...
| Classes, visibility blocks, constructors, destructors | Implemented in bootstrap compiler | Parser accepts `Class`, visibility blocks, constructors/destructors, nested classes, and methods; codegen lowers classes to C structs and methods to mangled `Class__Method(this, ...)` functions. |
...
| `New`/`Delete` | Implemented in bootstrap compiler/runtime | `New Class(...)` lowers to generated `Class__new[_N]` helpers using `CMotive_New` followed by constructor dispatch; `Delete obj` lowers to `Class__delete`, destructor dispatch, and `CMotive_Delete`. |
