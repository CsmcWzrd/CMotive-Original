...
| Classes/methods | Implemented in bootstrap compiler | Classes lower to concrete C structs; methods lower to `Class__Method(this, ...)` symbols; object and pointer method calls are rewritten to mangled calls. |
...
| Constructors/destructors | Implemented in bootstrap compiler | Constructors named as the class and `~Class` destructors lower to `Class__ctor[_N]` and `Class__dtor`; derived constructors/destructors chain to the base. |
| New/Delete | Implemented in bootstrap compiler/runtime | `New Class(...)` routes through `CMotive_New`, invokes the matching constructor, and returns `Class*`; `Delete obj` invokes the destructor and then `CMotive_Delete`. |
