...
def examples_root(root: Path) -> Path:
    return root / 'examples'


def load_rows(root: Path):
    manifest = examples_root(root) / 'manifests' / 'examples.jsonl'
...
def compile_one(root: Path, compiler: str, row: dict, out: Path, obj=False, keep_c=False):
    ...
    return run(compiler_cmd(compiler, args), timeout=60)


def preprocess_one(root: Path, pp: str, row: dict, out: Path):
    src = root / row['file']
    out.parent.mkdir(parents=True, exist_ok=True)
    args = include_args(root) + [str(src), '-o', str(out)]
    return run(compiler_cmd(pp, args), timeout=30)
...
def verify_run(root: Path, compiler: str, rows: list[dict], timeout: float, keep_build=False):
    build = Path(tempfile.mkdtemp(prefix='cmotive-merged-examples-'))
    failures = []
    try:
        for idx, row in enumerate(rows, 1):
            ...
            if idx % 25 == 0:
                print(f'run: {idx}/{len(rows)}')
        report = root/'examples'/'VERIFY_REPORT.md'
        if failures:
            report.write_text('# CMotive merged examples verification report\n\nFAILED\n\n```json\n'+json.dumps(failures, indent=2)+'\n```\n', encoding='utf-8')
            print(f'FAILED: {len(failures)} of {len(rows)} example(s). See {report}', file=sys.stderr)
            return 1
        report.write_text(f'# CMotive merged examples verification report\n\nPASS: {len(rows)} examples compiled, linked, executed, exited with expected status, and produced no stderr.\n', encoding='utf-8')
        print(f'run: {len(rows)} examples')
        return 0
    finally:
        if keep_build:
...
def debug_symbols_mode(root: Path, compiler: str, rows):
    ...
    for token in ['debug_level: 3', 'optimization: O2', 'StartPackage__ExampleDebugSymbol__Add']:
...
def main(argv=None):
    ...
    root = project_root()
    if ns.mode == 'clean':
        shutil.rmtree(root/'build'/'examples', ignore_errors=True)
        print('cleaned example build outputs')
        return 0
    rows = load_rows(root)
    ...
    if ns.mode == 'check':
        return manifest_check(root, rows) or preprocess_mode(root, pp, rows) or build_mode(root, compiler, rows, 'objects') or verify_run(root, compiler, rows, ns.timeout, ns.keep_build)
